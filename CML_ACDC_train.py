"""
CML_ACDC_train.py  v8.5 (class-adaptive alpha + unlabeled DBM-Dice 低阈值)
协同多学习半监督分割 — ACDC 数据集

══════════════════════════════════════════════════════════════════════
v8.4 → v8.5 修改清单
══════════════════════════════════════════════════════════════════════
  优化 1  Class-adaptive alpha（最直接提升 Myo Dice 的方向）
  ────────────────────────────────────────────────────────
  原版：boundary_weighted_ce_loss / masked_soft_dice_loss 对所有类别
    使用同一个标量 db_boundary_alpha（默认 2.0）。
    ACDC 的 Myo（心肌）是薄环形结构，宽度仅 3-6 像素，其整个结构
    几乎都在 sigma=3 的 SDF 边界带内，与 LV 内部大量像素用同一 alpha
    导致 Myo 边界约束相对不足，Dice 提升受限。

  修复：新增 --db_alpha_{bg,rv,myo,lv} 四个参数（默认 0.5/2.0/4.0/1.5），
    构造逐像素 alpha_map，对 Myo 施加 2× 强度约束，对背景降低噪声干扰。
    新增辅助函数 build_alpha_map / build_boundary_spatial_weight，
    向后兼容（不改动 decoder_boundary_module.py）。

  优化 2  无标签 DBM-Dice 置信度阈值独立化
  ────────────────────────────────────────────────────────
  原版：无标签路径 db_u_weight = db_rel_u * adap_w，其中 adap_w 使用
    conf_thresh=0.72，训练前期 adap_w≈0，DBM output-level Dice 对无标签
    数据几乎没有梯度贡献。

  修复：新增 --db_dice_conf_thresh（默认 0.45），对 DBM-Dice 专用更低阈值。
    边界 Dice 加权本身有选择性（只在 soft_boundary 高处有效），对噪声
    标签更鲁棒，不需要与 CE 损失共用同一严格阈值。
    新增 TensorBoard 指标 DB/dice_u_active_ratio 监控有效像素比例。

══════════════════════════════════════════════════════════════════════
v8.2 → v8.3 修改清单（Dice 核心 bug 修复）
══════════════════════════════════════════════════════════════════════
  BUG 1（严重）置信度加权公式错误 → Dice 不涨的根本原因
  ────────────────────────────────────────────────────────
  原版: ce * (1.0 + conf_weight * adap_w)，权重范围 [1.0, 2.0]
    低置信 / 模型不一致像素（adap_w≈0）仍有基础权重 1.0，
    其错误伪标签梯度始终参与反传。
    训练初期两模型大量不一致 → conf_map≈0 → adap_w≈0 → 所有像素
    权重≈1.0 → 错误梯度占主导 → Dice 停滞。

  修复: 改为乘法权重 adap_w * (1 + conf_weight)，范围 [0, 1+conf_weight]
    adap_w≈0 → CE 权重≈0，噪声像素被真正屏蔽
    adap_w≈1 → CE 权重≈(1+conf_weight)，高置信像素充分参与
    同时改为 sum/valid_pixels 归一化，避免 batch 中有效像素少时梯度缩水。

  BUG 2（严重）CPAM 使用未过滤 mixu_plab → 原型污染导致 Dice 停滞
  ────────────────────────────────────────────────────────
  原版 loss_proto_contrast / loss_discrepancy_compact 直接用 mixu_plab
    （含大量低置信错误标签），把无标签特征拉向错误原型方向。

  修复: 用 conf_mixu >= cpam_consistency_threshold 过滤，
    不满足的像素标记为 num_classes（ignore），CPAM 内部自动跳过。
    新增 TensorBoard 指标 train/cpam_ignore_ratio 监控过滤比例。

══════════════════════════════════════════════════════════════════════
v8.1 → v8.2 修改清单（DBM bug 修复，保留参考）
══════════════════════════════════════════════════════════════════════
  DBM 的 bug 修复（Bug A SDF 归一化、Bug B 伪标签对齐、Bug C ignore label、
  Bug D clamp）主要改善 HD95，对 Dice 无直接影响——DBM 是特征正则化器，
  不直接参与 CE 损失的优化路径。
══════════════════════════════════════════════════════════════════════
"""
