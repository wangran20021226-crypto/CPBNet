...
修复：
- EnhancedPrototypeContrastiveLoss: labels_v 强制转为 long，修复 cross_entropy Float类型报错
- NoiseAugmentedPrototypeLoss: 同上，labels_v 强制转为 long
"""
...
class DualModelPrototypeMemory:
    ...
    def update_with_consistency(self,
        ...
        # 批量累加（避免循环）
        reliable_pred_expanded = reliable_pred.long().unsqueeze(1).expand(-1, D)  # [M, D]
        ...
        weight_sum.scatter_add_(0, reliable_pred.long(), reliable_weights)
        ...
        for c in range(C):
            if valid_classes[c]:
                class_reliability = reliable_weights[reliable_pred.long() == c].mean()
...
class AdaptiveDiscrepancyCompactnessLoss(nn.Module):
    ...
    def forward(self,
        ...
        labels_flat = labels.reshape(N).long()  # 确保为long
...
class NoiseAugmentedPrototypeLoss(nn.Module):
    ...
    - 改进数值稳定性
    - 减少内存占用
    - 修复：labels_v 强制 .long()，避免 cross_entropy Float报错
    """
    ...
    def forward(self,
        ...
        # ★ 修复：强制转为 long，避免 cross_entropy 收到 Float 类型报错
        labels_v = labels_flat[valid_mask].long()
